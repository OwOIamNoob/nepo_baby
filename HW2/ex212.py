...
def show_img(origin, row_fft, row_col_fft):
    ...
    axs[2].set_title('2 Dimensional FFT')
    axs[2].axis('off')
    plt.savefig("HW2/outputs/fft.png")
    plt.show()
...
def DFT_2D(gray_img):
    ...
    row_fft = np.array([np.fft.fft(gray_img[i, :]) for i in range(h)])
    row_col_fft = np.array([np.fft.fft(row_fft[:, i]) for i in range(row_fft.shape[1])]).T
